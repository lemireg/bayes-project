...
### Likelihoood
Our data are denoted by $y_{ij}$ which represents the number of professional sports players. The subscripts $i,j$ indicate birth month and prefecture, respectively. Our likelihood, or data generating mechanism, is described by the following distribution:
...
### Priors
<!-- do we pluralize priors because we have multiple params or singular if it's a vector? -->
Our priors on $\lambda_{ij}$ are defined by the exponential relationship below where $\theta_{ij}$ is the total number of male children born in month $i$ in prefecture $j$. The $a_j$ intercept term is meant to capture the differences in the likelihoods between birthplaces.
