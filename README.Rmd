...
$$P(\lambda_i|\vec{y}) \propto \prod_{i=0}^{11} e^{-\lambda}\frac{\lambda_i^{y_i}}{y_i!} \times \theta_{i}e^{(\alpha +\beta RA_i)}$$
...
As presented in the previous section, our model is a simplification of the author's hierarchical model. We removed the model terms which provide random effects by prefecture (denoted $j$ in the original paper)
...
### Simulated Data for $y_i$
The author did not provide the data for the birth months of the 227 soccer players aged 23-25 in the 2012 season. However he did refer to the Japan Professional Football League (J. League) website which contain these data. Since we are interested in using simulated data we actually generate our $y_i$ values using the $\lambda_i$ values for different months **PROVIDED BY THE RESULTS IN THE PAPER**.
...
### Parameter Values for $\alpha_i$ and $\beta_i$
